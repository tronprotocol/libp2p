...
import org.tron.p2p.dns.update.DnsRoute53Publish;
import org.tron.p2p.dns.update.RecordSet;
...
public class Route53Test {
  ...
  public void testChangeSort() {
    ...
    existing.put("fdxn3sn67na5dka4j2gok7bvqi.n",
        new RecordSet(new String[] {"enrtree-branch:"}, DnsRoute53Publish.treeNodeTTL));
    ...
    DnsRoute53Publish publish = new DnsRoute53Publish("random1", "random2", "random3",
        Region.US_EAST_1);
    ...
    for (int i = 0; i < changes.size(); i++) {
      ...
      Assert.assertTrue(DnsRoute53Publish.isSameChange(wantChanges[i], changes.get(i)));
    }
  }
  ...
  public void testPublish() throws UnknownHostException {
    ...
    DnsRoute53Publish dnsRoute53Publish = new DnsRoute53Publish("AKIA5R2IGLJ7POSJ7TOO",
        "j69OXFi0RD+0k1yrqsnKU8BFYpPAvtMds+eMUcAu",
        "",
        Region.US_EAST_1);

    dnsRoute53Publish.deploy("nftderby1.net", tree);
    ...
    Tree route53Tree;
    try {
      route53Tree = client.syncTree(Entry.linkPrefix + base32Pubkey + "@nftderby1.net");
    } catch (Exception e) {
      Assert.fail();
      return;
    }
