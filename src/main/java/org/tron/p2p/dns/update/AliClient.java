...
public class AliClient {
  ...
  private final long rootTTL = 30 * 60;
  private final long treeNodeTTL = 4 * 7 * 24 * 60 * 60;
  ...
  public void deploy(String domainName, Tree t) throws DnsException {
    ...
    try {
      Map<String, DescribeDomainRecordsResponseBodyDomainRecordsRecord> existing = collectRecords(domainName);
  ...
  public boolean deleteDomain(String domainName) throws Exception {
    ...
    DeleteSubDomainRecordsResponse response = client.deleteSubDomainRecords(request);
    return response.statusCode == successCode;
  }

  public Map<String, DescribeDomainRecordsResponseBodyDomainRecordsRecord> collectRecords(String domain)
      throws Exception {
    ...
    try {
      ...
      while (true) {
        ...
        if (response.statusCode == successCode) {
          for (DescribeDomainRecordsResponseBodyDomainRecordsRecord r : response.getBody().getDomainRecords().getRecord()) {
  ...
  private void submitChanges(String domainName,
    ...
    for (Map.Entry<String, String> entry : records.entrySet()) {
      ...
      if (!existing.containsKey(entry.getKey())) {
        ...
        result = updateRecord(existing.get(entry.getKey()).getRecordId(), entry.getKey(), entry.getValue(), ttl);
      }
